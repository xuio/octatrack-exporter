...
Scale policy (user requirement): PER TRACK mode with MASTER LENGTH = INF (master_len_per_track_multiplier=255 AND master_len_per_track=255 per ot-tools-io) and master_scale_per_track left at 1x (code 2); pattern length comes ONLY from each track's SCALE TRACK len + TEMPO MULTIPLIER (ATT+89/+90). The pattern-level scale block is pattern-global and is always written whole — without scale_mode=1 per-track mode never engages — but ATT+89/+90 is written ONLY on tracks that carry an exported stem (writeBankPatterns' `scaleTracks` argument, computed in projectBuild from the exported stems' slots). Reason: the user keeps track 7 for remixing at a 1-bar scale of their own, and track 8 is their master track — a stem is never allowed to restamp either, so track index 7 is excluded even when 8 stems are loaded (its trigs are still written; a report warning tells the user to set track 8's scale by hand). Stem tracks DO get the scale in every programmed pattern, including ones where that stem has no trig, or the loop would not match the section. Readback checks LEN/mult only on scaleTracks; the untouched tracks are proven byte-identical across a write by the sentinel tests in tests/writers.test.js and tests/writer-guards.test.js instead.
...
# Readback verification (src/lib/readback.js)
Every one of the three format bugs above (slice ×2, mask byte order, the +12 dB gain) looked correct
in the writer and was only caught by playing the export on the device. The readers in readback.js
decode the generated bytes again and verifyExport() diffs that decode against the pattern table the
export was built from; buildProject() runs it on every project and reports the result.
- Independently derived: the readers re-express their own field offsets and their own inverse of the
  half-page reversal (step n → file byte 7 - ((n-1)>>3)) rather than importing the writer's. A change
  made on one side and not the other is the thing this is for.
- Shared on purpose: bankLayout(), which finds the PTRN/TRAC sections by magic bytes in the user's
  own file. That is measurement, not an assumption, and both sides must agree on which bytes they
  are even looking at.
- What it proves: the writers did what the build asked. What it cannot prove: that OSSC and the
  device agree on what a byte means. Only a device can settle that.
- Also catches the writers' silent `continue` paths — a job whose LEN the bank writer refuses would
  otherwise ship as a section with no trigs and no warning.
